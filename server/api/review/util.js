...
function getStudyOverviewStats(study) {
    ...
    var taskAnswers = _(study.answers).filter(function (answer) {
        return answer.questionType === 'Task' && !_.isUndefined(answer.answer);
    }).value();
    ...
    Study.findOne({
        ...
        .exec(function (err, study) {
            if (err) {
                ...
                serviceLogger.info('>> getStudyAndOverviewStats(), no study found');
...
function getIndividualQuestionWithStats(question, answers, annotations, studyDetails, breakdownRequired) {
    ...
    promiseChain = getTrackingForQuestion(studyDetails.projectId, studyDetails._id, question._id)
        .then(function (trackings) {
            ...
            // carry over the trackings to the next block so the average time for a question
            // can be calculated correctly
            questionWithStats.trackings = trackings;
            return getAnnotationsForQuestion(question._id, annotations);
        });

    // question
    promiseChain.then(function (annots) {
        var uniqueParticipants = getUniqueParticipants(questionAnswers, annots);
        questionWithStats.participants = uniqueParticipants.length;
        questionWithStats.averageDuration = getAverageTimeSpentOnQuestion(questionWithStats.trackings, question._id.toString(), uniqueParticipants);
        // remove the trackings from the returned object
        delete questionWithStats.trackings;

        questionWithStats.annotations = annots.length;
        questionWithStats.comments = _(annots)
            .filter(function (annotation) {
                return annotation.questionId.equals(question._id) && annotation.comment && annotation.comment.length > 0;
...
function getChoiceStats(answerItems, question) {
    ...
    // Step 1. Break out the answers selected by each user, these need to be formatted correctly for the UI i.e. Q1, Q2
    var answers = [];
    var answerOptions = question.answerOptions;
    answerItems.forEach(function (answer) {
        var answerSplit = [].concat(answer.answer.split(ANSWER_SEPARATOR));
        answers = answers.concat(answerSplit);
        answer.answerText = getAnswerTextForMultiChoice(answerSplit, answerOptions).toString();
    });

    // Step 2. Create default representing the choices available for the question
    var choicesObj = {};
    // pair with the answer options
    answerOptions.forEach(function (choice, index) {
        choicesObj[index] = {
            questionText: choice,
            value: 0
        };
    ...
    // Step 3. Iterate over all selected choices and increment count per item
    answers.forEach(function (answer) {
        choicesObj[answer].value = choicesObj[answer].value + 1;
    });

    // Step 4. Calculate percentages for each chosen answer
    serviceLogger.info('<< getChoiceStats(), return data');
    return Object.keys(choicesObj).map(function (index) {
        return {
            text: choicesObj[index].questionText,
            total: choicesObj[index].value,
            percentage: (choicesObj[index].value === 0 ? choicesObj[index].value : Math.round(choicesObj[index].value / answers.length * 100))
        };
    });
}
...
function buildSankey(aTrackings, aDeepLinks, aTargetUrls) {
    ...
    var createBranchFromList = function (aList, oNode) {
        ...
        if (aList[0].context) {
            nodeKey = updateRelativePathWithContext(nodeKey, aList[0].context, {snapshotUILang: 'UI5'});
        }
    ...
    var checkPath = function (aList, oNode) {
        ...
        /**
         * Check for if the page has context. Only prototypes with navigation with entity and view (eg UI5, ST) should
         * have this context added to the node key, as these reflect different pages.  HTML prototypes should not.
         */
        if (aList[0].context) {
            nodeKey = updateRelativePathWithContext(nodeKey, aList[0].context, {snapshotUILang: 'UI5'});
        }
    ...
    var transformData = function (oNode, sKey, iParentNumber) {
        ...
        aNodes.push({
            ...
            name: oLink.displayName || oLink.pageName,
 ...
 * @param {array} answers
 * @param {array} annotations
 * @returns {array} unique user ids
 */
function getUniqueParticipants(answers, annotations) {
        ...
        .then(function (deepLinks) {
            if (deepLinks && deepLinks.length) {
                ...
                _.map(deepLinks, function (deepLink) {
                    ...
                    link.defaultContext = deepLink.defaultContext;
                    link.name = deepLink.displayName || deepLink.pageName; // html protos only have pageName
 ...
 * @param {array} task - (task object) needed to get the task UI Language
 * @returns {array}
 */
function getClickLocationsForUrl(sUrl, oTrackings, task) {
    var aClicks = _(oTrackings)
        .filter(function (track) {
            // occurrences of track without clicks - filter those out too
            if (track.context) {
                return updateRelativePathWithContext(track.pathName, track.context, task) === sUrl && (track.clickX || track.clickY);
            }
...
function getAnnotationUrl(annotation, task) {
    ...
    if (annotation.context) {
        ...
        if (annotation.context.entity || annotation.context.view) {
...
function getAnnotationStatsForUrl(url, annotations, task) {
    ...
    var comments = _.filter(annotations, function (annotation) {
        return taskId === annotation.questionId.toString() && getAnnotationUrl(annotation, task) === url && annotation.comment && annotation.comment.length > 0;
    });
...
function updateRelativePathWithContext(path, context, task) {
    if (context && task.snapshotUILang === 'UI5') {

        if (context.view && context.entity && context.view !== '' && context.entity !== '') {
            path += HASH_FORWARDSLASH + context.view + '/' + context.entity;
        } else if (context.view && context.view !== '') {
            path += HASH_FORWARDSLASH + context.view;
        } else if (context.entity && context.entity !== '') {
 ...
 * @returns Number
 */
function getAverageTimeSpentOnQuestion(trackings, questionIdStr, userList) {
    ...
    var questionTrackings = _.filter(trackings, function (tracking) {
        return tracking.questionId.toString() === questionIdStr && tracking.eventType !== 'iframeClick' && userList.indexOf(tracking.user.toString()) !== -1;
    });
    ...
    return Math.floor(totalTime / userList.length);
}
...
function getUsersWhoCompletedAStudy(study) {
    ...
    if (questionTypes.MultipleChoice || questionTypes.Freeform) {
    ...
    // initialize result to be an empty list for the case we just have one sublist in
    // usersCompleted and the study wasn't completed.
    var resultingUsers = [];

    // if usersCompleted just holds one sublist and there is just one questionType,
    // return the sublist. _.intersection is not returning the single list if we
    // just pass it in as single argument.
    if (usersCompleted.length === 1 && Object.keys(questionTypes).length === 1) {
        resultingUsers = usersCompleted[0];
    }
    else if (usersCompleted.length > 1) {
        // get the intersection of all sublists for the question types
        // to determine which users completed all question types
        resultingUsers = _.intersection.apply(this, usersCompleted);
    }
            ...
            if (err) {
                ...
                _.each(participants, function (participant) {
                    ...
                    if (tracks) {
                        ...
                        _.each(participant.tracks, function (tracking) {
    ...
    Tracking.find({
        ...
        _.each(userNavTrackings, function (userTrackings) {
            ...
            if (userTrackings.length > 1) {
                //participant navigate to multiple pages on the task, tracking stats for each pages and summarize all stats
                participant =
                ...
                var relevantAnnotations = filterTaskAnnotationsByUser(studyDetails.annotations, participant._id, taskId);
                var relevantComments = filterTaskAnnotationsWithComments(relevantAnnotations);
                ...
                aggregatedTrackings.push(participant);
                 ...
                 //participant navigate to a single page on the task, tracking stats for a single page.
                ...
                var totalAnnotations = _.filter(studyDetails.annotations, function (annotation) {
                    return annotation.url === userTrackings[0].pathName && annotation.createBy.equals(userTrackings[0].user) && (String(annotation.questionId) === String(taskId));
                }).length;
                var totalComments = _.filter(studyDetails.annotations, function (annotation) {
                    return annotation.url === userTrackings[0].pathName && annotation.createBy.equals(userTrackings[0].user) && annotation.comment && annotation.comment.length > 0 && (String(annotation.questionId) === String(taskId));
...
function getNumberOfComments(annotations, trackingEntry) {
    return _.filter(annotations, function (annotation) {
        ...
        return annotation.comment && annotation.comment.length > 0 && annotation.url === trackingUrl;
    }).length;
}
...
function getVisitsForTaskUrl(trackings, pageUrl, task) {
    return _.filter(trackings, function (track) {
            if (track.context) {
                return (updateRelativePathWithContext(track.pathName, track.context, task) === pageUrl) && (track.questionId.toString() === task._id.toString());
            }
 ...
 *
 * @param {array} answers - list of indices of the options selected.
 * @param {array} aChoices - The list of possible options
 * @returns {array} answers - array of strings containing the actuall answers
 */
function getAnswerTextForMultiChoice(answers, choices) {
    var answersList = [];
    answers.forEach(function (answer) {
        answersList.push(choices[answer]);
    });
    return answersList;
}
 ...
 */
function filterTaskAnnotationsByUser(annotations, userId, taskId) {
    return _.filter(annotations, function (annotation) {
        return String(annotation.createBy) === String(userId) && (String(annotation.questionId) === String(taskId));
    });
}
 ...
 */
function filterTaskAnnotationsWithComments(annotations) {
    return _.filter(annotations, function (annotation) {
        return annotation.comment && annotation.comment.length > 0;
    });
}
...
exports.getUniqueParticipants = getUniqueParticipants;
