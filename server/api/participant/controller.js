        ...
        .exec(function (err, study) {
            ...
            assetService.getAsset(assetId, versionId, (req.query.thumbOnly === 'true'))
                .then(function (asset) {
                    ...
                    return utils.sendError(res, err2);
                });
        });
};
...
exports.participate = function (req, res) {
    ...
    Study.find({
        ...
        .exec(function (err, studies) {
            ...
            serviceLogger.info('participate(), study list, size ' + studies.length);

            //  find studies where user has been invited and did not participate in the studies
            getInvitedStudies(req.user)
            .then(function (invitedStudies) {
                //  add invited studies to study list
                var studiesWithInvitedStudies = studies.concat(invitedStudies);
                serviceLogger.info('participate(), study list include invited studies, size ' + studiesWithInvitedStudies.length);
                return res.status(200).json(studiesWithInvitedStudies.map(function (study) {
                    ...
                    //  if user is invited and not participant in the study, then mark hasInviteed to true
                    if (study.invite_list) {
                        study.hasInvited = true;
                        study.participatedAt = null;
                        study.invite_list = undefined;
                    }
                    else {
                        study.hasInvited = false;
                    }

                    // We don't want participants to know who else took the study
                    study.participants = undefined;

                    serviceLogger.warn('<< participate(), returning study');
                    return study;
                }));
            });
...
/**
 * Get list of the studies where user has been invited to and did not participate in the study.
 * @param  {Object}  User object
 * @return {Object}  study list
 */
function getInvitedStudies(user) {
     serviceLogger.info({
        user: user
    }, '>> getInvitedStudies()');
    ...
    Study.find({
        ...
        'invite_list.email': user.email,
        deleted: false,
        $and: [{$or: [{status: 'published'}, {status: 'paused'}]}]
    })
    .select('name description createBy projectId createTime questions answers annotations participants deleted status invite_list')
    .lean()
    .exec(function (err, studies) {
        if (err) {
            serviceLogger.warn('<< getInvitedStudies(), returning error, ' + err);
            deferred.reject(err);
        }
        else {
            serviceLogger.info('<< getInvitedStudies(), returning invited studies, size ' + studies.length);
