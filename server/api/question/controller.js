...
exports.update = function (req, res) {
    ...
    if (req.body.answerLimit) {
        ...
        updatedQuestion['questions.$.answerLimit'] = null;
    }
...
exports.uploadFiles = function (req, res) {
    ...
    if (req.files && req.files.file) {
        ...
        // If the request contains multiple files, the first is an image and the second is the thumbnail
        if (Array.isArray(file) && (ACCEPTED_FILE_TYPES[file[0].extension.toLowerCase()] === 'image')) {
            handleImages(req, res);
        }
        else if (file.extension && ACCEPTED_FILE_TYPES[file.extension.toLowerCase()] === 'archive') {
    ...
    // no files
        ...
        serviceLogger.error('<< uploadFiles(), no files received in request', req.files);
        return res.status(422).json({
