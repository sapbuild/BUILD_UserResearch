...
module.exports = function ($scope, $state, $stateParams, $window, $document, $timeout, $http, $log,
                           zip, uiThumbnailGenerator, Snapshots, Tasks, TaskCreator, urUtil) {
    ...
    $scope.fileInputChange = function (file) {
        $scope.init(true);
        $scope.uploadCancelled = false; // reset flags as this is a new upload
        $scope.processing = true;
        util.getEntries(zip, file, parseEntries, $scope.handleError);
    };

    /**
     * Callback for the invalid-files callback (file-upload control)
     * Shows error to user
     */
    $scope.invalidFilesSelected = function () {
        taskCreator.showInvalidError();
    };
    ...
    $scope.onSnapshotSelect = function (prototype) {
        ...
        $scope.pages.forEach(function (oPage) {
            oPage.selected = true;
        });
    };
