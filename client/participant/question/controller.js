...
module.exports = function ($window, urUtil, $log, $scope, $state, $stateParams, $timeout, $location,
    ...
    $scope.action = 'close'; // used for css animations;
    ...
    // Attaching backForwardHandler to the $rootScope if it hasn't already been added
    if ($rootScope.$$listeners.$locationChangeSuccess.length === 1) {
    ...
    /**
     * Handler Used to detect if the browser back/forward events are used and sets the appropriate animation action
     * @oaram newLocation - The url the event has brought the app to.
     * @oaram oldLocation - The url the event has brought the app from.
     */
    function backForwardHandler(newLocation, oldLocation) {
        if ($scope.actualLocation === newLocation) {
            if (oldLocation.contains('question') === false) {
                // It is coming from the list page
                $scope.action = 'open';
            } else if (newLocation.contains('question') === false) {
                // It is going to a list page
                $scope.action = 'close';
            } else {
                for (var i = 0; i < $scope.study.questions.length; i++) {
                    if (oldLocation.contains($scope.study.questions[i]._id)) {
                        // It is coming from a previous question
                        $scope.action = 'next';
                        break;
                    }
                    if (newLocation.contains($scope.study.questions[i]._id)) {
                        // It is coming from a further question
    ...
    // Need to watch the $location.path in order to know when back/forward events are being sent
    ...
    $scope.endTask = function (finished) {
        ...
        var status;
        // We don't need to store the data of the url in the answer
        $scope.currentAnswer.answer = $scope.iframeLoc.split('(')[0];
    ...
    $scope.handleMessage = function (event) {
        if (urUtil.verifyIframe('prototype-iframe', event) === true) {
            if (event.data.type === 'iframeOnload') {
                ...
                $scope.iframeLoc = urUtil.getRelativeURI(event.data.newLocation);
                $scope.currentUrl = $scope.iframeLoc;
    ...
    $scope.allowedToDropAnnotations = function () {
        return !($scope.currentQuestion.answerIsLimited && typeof $scope.currentQuestion.answerLimit !== 'undefined' && $scope.currentQuestion.answerLimit === $scope.annotations.length);
    };
