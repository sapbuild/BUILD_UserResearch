...
module.exports = function ($compile, $timeout) {
    ...
    return {
        ...
        link: function (scope, elem) {
            ...
            scope.animationListener = function () {
                $timeout(function () {
                    var newDimensions = scope.imageContainer[0].getBoundingClientRect();
                    if (newDimensions.width !== scope.containerDimensions.width || newDimensions.height !== scope.containerDimensions.height) {
                        scope.resizeImage();
                        scope.animationListener();
                    } else {
                        elem.addClass('resized');
                    }
                }, 100);
            };
