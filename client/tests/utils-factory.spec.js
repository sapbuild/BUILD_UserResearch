...
describe('Unit tests for UserResearch utils', function () {
    ...
    it('should get context from url of UI5 SmartApp', function () {
        var testUrl = 'https://build-master-ci.mo.sap.corp:9443/api/projects/f274ca6fbb388e980a567531/prototype/snapshot/latest/index.html#/SalesOrder(\'pears\')';
        var context = util.getContextFromUrl(testUrl, 'UI5');
        expect(context.context_type).to.equal('UI5');
        expect(context.entity).to.equal('SalesOrder');
        expect(context.view).to.equal('');
        expect(context.data).to.equal('pears');
    });

    it('should get normal context from non SmartApp UI5 URL', function () {
        var testHash = 'https://build-master-ci.mo.sap.corp:9443/api/projects/f274ca6fbb388e980a567531/prototype/snapshot/latest/index.html#/S1';
        var context = util.getContextFromUrl(testHash, 'UI5');
        expect(context.context_type).to.equal('UI5');
        expect(context.view).to.equal('S1');
        expect(context.entity).to.equal('');
        expect(context.data).to.equal.undefined;
    });

    it('should handle navigation with context appropariately', function () {
        var testHash = 'https://build-master-ci.mo.sap.corp:9443/api/projects/f274ca6fbb388e980a567531/prototype/snapshot/latest/index.html#/S1/SalesOrder(\'pears\')';
        var context = util.getContextFromUrl(testHash, 'UI5');
        expect(context.context_type).to.equal('UI5');
        expect(context.view).to.equal('S1');
        expect(context.entity).to.equal('SalesOrder');
    ...
    it('should handle navigation with context appropariately with multiple entities', function () {
        var testHash = 'https://build-master-ci.mo.sap.corp:9443/api/projects/f274ca6fbb388e980a567531/prototype/snapshot/latest/index.html#/S1/SalesOrder(\'pears\')/SalesOrderItem(\'oranges\')';
        var context = util.getContextFromUrl(testHash, 'UI5');
        expect(context.context_type).to.equal('UI5');
        expect(context.view).to.equal('S1');
        expect(context.entity).to.equal('SalesOrderItem');
        expect(context.data).to.equal('oranges');
    });
