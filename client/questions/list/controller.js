...
module.exports = function ($scope, $state, $filter, $rootScope, $timeout, Questions, Tasks, currentStudy, uiError, TaskCreator, urUtil) {
    ...
    $scope.addQuestions = function (questions) {
        ...
        // Ordinal is incremented per new question
    ...
    $scope.addUploadedItem = function (responseDetails, sequence) {
        ...
        if (responseDetails.appMetadata) {
            ...
            taskData.task.then(function (task) {
                ...
                // we need to do this because the thumbnail directive captures the url
                // and makes a request based on it but won't request if the URL is undefined/null
                // the thumbnail is set back in the thumbnails callback.
                // we still need the task so that it appears to be processing.
                task.tempThumbnail = task.thumbnail;
                task.thumbnail = null;
                oTempTask = task;
                $scope.study.questions = $scope.study.questions.concat([oTempTask]);
                $scope.init();
            });

            // handlers for the processing progress of the thumbnail generation & upload
            taskData.thumbnails.then(function success(data) {
                    // thumbnail is actually uploaded at this stage.
                    data.uploadThumbnails.then(function () {
                        $scope.aProcessingTasksProgress[oTempTask._id] = 100;
                        // because we added this dud one remove it and replace again
                        _.remove($scope.study.questions, function (question) {
                            return oTempTask._id === question._id;
                        });
                        oTempTask.thumbnail = oTempTask.tempThumbnail;
                        oTempTask.tempThumbnail = null;
                        $scope.study.questions = $scope.study.questions.concat([oTempTask]);
                    });
                },
                function error() {
    ...
    /**
     * Callback for upload-failure from file-upload control
     * Reset isUploadingScreen to false and hide the upload tiles.
     */
    $scope.onUploadFailure = function (err) {
        ...
        $scope.isUploadingScreen = false;
        $rootScope.$broadcast('image-uploaded');
        urUtil.showUiError('Error uploading files', err);
    };
    ...
    $scope.rejectFiles = function () {
        urUtil.showUiError('It is only possible to upload image files and prototype zip files to this study.');
    };

    $scope.rejectFilesZipUpload = function () {
        urUtil.showUiError('It is only possible to upload .zip here');
    };
    ...
    $scope.updateOrder = function () {
        ...
        for (; screen = screenList[i]; i++) {
            ...
            var screenQuestions = _.filter($scope.study.questions, function (filteredQuestion) {
            ...
            // Update the ordinal per question
        ...
        // Update questions with latest ordinal values
