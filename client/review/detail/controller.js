...
// @ngInject
module.exports = function ($state, $stateParams, $scope, $rootScope, $window, $timeout, $log, SENTIMENT, urUtil, Reviews, uiTabHelper, Studies, scrollToCommentApi) {
    ...
    var currentStudy = $stateParams.study;
    var currentQuestion = $stateParams.questionId;
    ...
    $scope.initDetail = function (curStudy, curQuestionId) {
        ...
        for (var i = 0; i < curStudy.questions.length; i++) {
            if (curStudy.questions[i]._id === curQuestionId) {
                ...
                if (i === 0) {
                    $scope.prevQId = curStudy.questions[curStudy.questions.length - 1]._id;
                }
                else {
                    $scope.prevQId = curStudy.questions[i - 1]._id;
                }
        ...
        if (typeof $scope.showAnnotations === 'undefined') {
         ...
         $scope.qAnnotations = currentStudy.annotations ? currentStudy.annotations.filter(function (annotation) {
            if (annotation.questionId === $scope.question._id) {
                if (($scope.question.snapshotUILang === 'UI5') || (!$scope.prototypeUrl || annotation.url.split('#')[0] === $scope.prototypeUrl.split('#')[0])) {
                    if ($scope.question.snapshotUILang === 'UI5') {
                        $scope.current_context = urUtil.getContextFromUrl($scope.prototypeUrl || $scope.question.url, 'UI5');
                    }
    ...
    $scope.initStudy = function (questionId) {
        if (questionId) {
            currentQuestion = questionId;
            if (!currentStudy) {
                currentStudy = Studies.get({
            ...
            if (currentStudy.$promise) {
                currentStudy.$promise.then(function () {
                    $scope.initDetail(currentStudy, questionId);
                });
            }
            else {
                $scope.initDetail(currentStudy, questionId);
    ...
    $scope.$on('init-review-details', function (even, data) {
        delete $scope.prototypeUrl;
        delete $scope.showAnnotations;
        $scope.initStudy(data.questionId);
    });
    ...
    $scope.goBack = function () {
        // force the image-toggle to re-render the image when the popup is reopened
        $scope.question = null;
        $scope.statistics = null;

        $timeout(function () {
            $state.go('^');
            $rootScope.$broadcast('dialog-close', 'review-detail-modal');
        });
    };
    ...
    $scope.$watch('topPanelSelectedTab', function (val) {
        // if we are not in the prototype tab, show all annotations in RHS pane.
        if (val !== 'prototype-tab') {
    ...
    /**
     *  Store the initial length of the page history.
     *  This value is used in the iframe to determine whether it can go back or not
        ...
        if (urUtil.verifyIframe('prototype-iframe', event) === true) {
            if (event.data.type === 'iframeOnload' || event.data.type === 'iframeHashchange') {
                ...
                if (typeof $scope.location !== 'undefined') {
                    ...
                    $scope.initDetail(currentStudy, currentQuestion);
                ...
                // we need to calculate iFrame annotations separately to consider the scroll
