...
describe('QuestionsTasks Tests', function () {
    this.timeout(30000);
    var mainTestUserId;
    var mainTestUserNameEmail = 'userOne_' + new Date().getTime() + '@sap.test';
    var userTwoId;
    var userTwoNameEmail = 'userTwo_' + new Date().getTime() + '@sap.test';
    ...
    it('Should not be able to add a new question with wrong studyId', function (done) {

        QuestionApi.createQuestion(mainTestUser, projectId, null, question2, 500)
            .then(function (res) {
    ...
    it('Should be able to update a question', function (done) {
     ...
     it('Should not be able to get study with invalid studyId', function (done) {
        expect(studyId).not.to.be.empty;
        var invalidStudyId = studyId + '1234';
        StudyApi.getStudy(mainTestUser, projectId, invalidStudyId, 500)
            .then(function (res) {
    ...
    it('Should not be able to update order of questions question with emptyQuestions', function (done) {
        var updatedQuestions = [];
        QuestionApi.updateMultipleQuestion(mainTestUser, projectId, studyId, {  questions: updatedQuestions }, 400)
            .then(function (res) {
                expect(res.body.error).to.exist;
                done();
            })
    ...
    it('Should not be able to update order of questions question with invalid StudyId', function (done) {
        var updatedQuestions = [];
        var invalidStudyId = studyId + '1234';
        QuestionApi.updateMultipleQuestion(mainTestUser, projectId, invalidStudyId, {  questions: updatedQuestions }, 500)
            .then(function (res) {
                expect(res.body.message).to.exist;
                expect(res.body.name).to.equal('CastError');
                done();
    ...
    it('Should be able to upload a zip file', function (done) {
        QuestionApi.uploadFiles(mainTestUser, projectId, path.resolve(__dirname, 'TestingDriveHeader.zip'), 200)
            .then(function (res) {
                done();
            })
            .catch(done);
      ...
      it('Should not be able to upload a image file with non support file type', function (done) {

        QuestionApi.uploadFiles(mainTestUser, projectId, path.resolve(__dirname, 'Large.unsupport'), 422)
            .then(function (res) {
                done();
            })
            .catch(done);
