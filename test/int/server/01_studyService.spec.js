...
var USER_EMAIL_ONE = 'studyService_' + new Date().getTime() + '@sap.test';
var USER_EMAIL_TWO = 'studyService_' + new Date().getTime() + '@sap.test';
var remember;
...
describe('StudyService REST API Test', function () {
    ...
    before('Initialize User One', function (done) {
        mainTestUser.initialize(USER_EMAIL_ONE, 'Minisap!1').then(function () {
    ...
    before('Initialize User Two', function (done) {
        mainTestUser.initialize(USER_EMAIL_TWO, 'Minisap!1').then(function () {
            return api.createProject(mainTestUser, {name: PROJECT_NAME}, 201);
        }).then(function (res) {
            projectId = res.body._id;
            done();
    ...
    it('Should create a new study, created by the logged in user', function (done) {
    ...
    it('7 - Should be able to update existing study', function (done) {
        ...
        api.updateStudy(mainTestUser, projectId, studyId, {
            ...
            .then(function (res) {
                ...
                expect(res.body.publishTime).not.to.be.empty;
                expect(res.body.questions).to.be.an.instanceof(Array);
                expect(res.body.questions.length).to.equal(1);
                studyId = res.body._id;
                expect(studyId).not.to.be.empty;
                lastPublishTime = res.body.publishTime;
                done();
            })
            .catch(done);
    });
     ...
     */
    it('12 - Should be able to upload a zip file to create a new task', function (done) {
        api.uploadZip(mainTestUser, projectId, path.resolve(__dirname, 'TestingDriveHeader.zip'), 200)
            .then(function (res) {
                console.log('12 - uploadZip ');
                expect(res.body.projectId).to.equal(projectId);
                expect(res.body.appMetadata.model.uiLang).to.equal('html');
                expect(res.body.snapshot.deepLinks.length).to.equal(3);
                expect(res.body.snapshot.snapshotUILang).to.equal('html');
                done();
            })
            .catch(done);
    });
    ...
    it('19 - Should be able to update status from paused to published.', function (done) {
         api.updateStudy(mainTestUser, projectId, studyId, {status: 'published'}, 200)
            .then(function (res) {
                ...
                expect(res.body.status).to.equal('published');
                expect(res.body.publishTime).to.be.above(lastPublishTime);
