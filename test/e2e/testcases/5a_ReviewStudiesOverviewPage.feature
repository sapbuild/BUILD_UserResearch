...
Feature: Review Study Overview Page
    ...
    Scenario: Click into Active Studies
    ...
    Scenario: Review the Totals in the Overview Page:
        ...
        And     I check the annotation count is "36"
        And     I check the comment count is "36"
        And     I check the "Positive" sentiment is "9 33.33%" answers
        And     I check the "Positive" sentiment percentage is "33.33%"
        And     I check the "Nuetral" sentiment is "9 33.33%" answers
        And     I check the "Nuetral" sentiment percentage is "33.33%"
        And     I check the "Negative" sentiment is "9 33.33%" answers
        And     I check the "Negative" sentiment percentage is "33.33%"
        And     I check complete tasks is "2"
        And     I check task "Success" rate is "2 33.33%"
        And     I check task "Success" percentage is "33.33%"
        And     I check task "Failure" rate is "3 50%"
        And     I check task "Failure" percentage is "50%"
        And     I check task "Abandoned" rate is "1 16.67%"
        And     I check task "Abandoned" percentage is "16.67%"
