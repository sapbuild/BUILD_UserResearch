...
module.exports = function() {
    ...
    this.Then(/^Review Filters Exist$/, function (callback) {
       expect(review.reviewFilters.count()).to.eventually.equal(3).and.notify(callback)
    });

    this.When(/^I Click "([^"]*)" Filter$/, function (filter, callback) {
        var index = 0;
        if(filter==="Tasks"){
            review.clickFilter(index+1);
        }else if(filter==="Questions"){
            review.clickFilter(index+2);
        }else{
            review.clickFilter(index);
        }
        callback();
    });
    ...
    this.Then(/^QuestionTask "([^"]*)" Annotations dropped equals "([^"]*)"$/, function (quest, num, callback) {
        var id = parseInt(quest)-3;//-2 for index location correction
    ...
    this.Then(/^Task "([^"]*)" Target Reached equals "([^"]*)" with percentage "([^"]*)"$/, function (index,num, perc, callback) {
        var concat = num+" "+perc
        var loc = (parseInt(index)-1);
        expect(review.questionSuccessTotal.get(loc).getText()).to.eventually.equal(concat).and.notify(callback);
    });

    this.Then(/^Task "([^"]*)" Target Abandoned equals "([^"]*)" with percentage "([^"]*)"$/, function (index,num, perc, callback) {
        var concat = num+" "+perc
        var loc = (parseInt(index)-1);
        expect(review.questionAbandonedTotal.get(loc).getText()).to.eventually.equal(concat).and.notify(callback);
    });

    this.Then(/^Task "([^"]*)" Target Not Reached equals "([^"]*)" with percentage "([^"]*)"$/, function  (index,num, perc, callback) {
        var concat = num+" "+perc
        var loc = (parseInt(index)-1);
        expect(review.questionFailedTotal.get(loc).getText()).to.eventually.equal(concat).and.notify(callback);
    });
    ...
    this.Then(/^Stats Abandoned equals "([^"]*)" with percentage "([^"]*)"$/, function (num, perc, callback) {
        var concat = num+" "+perc
        expect(review.statsparticipantnumberabandoned.getText()).to.eventually.equal(concat).and.notify(callback);
    });
    ...
    this.Then(/^I see the Task Statstics Task Abandoned equals "([^"]*)" with percentage "([^"]*)"$/, function (num, perc, callback) {
        var concat = num+" "+perc
        expect(review.statsabandonedtotal.getText()).to.eventually.equal(concat).and.notify(callback);
    });
