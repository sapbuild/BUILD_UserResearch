...
module.exports = function() {
    ...
    this.When(/^I "([^"]*)" the Task$/, function (action,callback) {
        browser.waitForAngular();
        if(action==="Finish") {
            studyPage.clickFinish();
        }else{
            studyPage.clickAbandonTask();
            browser.waitForAngular();
            studyPage.clickConfAbandonTask();
        }
        browser.waitForAngular();
        callback();
    });
    ...
    this.Then(/^I see the abandoned text$/, function (callback) {
        browser.waitForAngular();
        expect(studyPage.abandoned.isEnabled()).to.eventually.equal(true).and.notify(callback);
    });
    ...
    this.Given(/^I am on a study Participant screen enlarge page$/, function (callback) {
        browser.waitForAngular();
        expect(studyPage.participantProgBar.isEnabled()).to.eventually.equal(true).and.notify(callback);
    });
