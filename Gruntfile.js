...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        // JS Inspect
        jsinspect: require('./grunt-conf/jsinspect.js'),
    ...
    grunt.registerTask('serve', function (target) {
        var tasks = {
            debug: ['build', 'env:dev', 'express:dev', 'node-inspector', 'watch'],
            dev: ['build', 'env:dev', 'express:dev', 'watch']
        };
    ...
    grunt.registerTask('test', function (target) {
        var tasks = {
            client: ['build:test', 'env:dev', 'eslint:client', 'karma', 'jsinspect:client', 'copy:testResult'],
            server: ['env:dev', 'eslint:server', 'mocha_istanbul', 'jsinspect:server'],
            e2e: ['express:dev', 'wait:dev', 'protractor', 'wait:dev', 'trimJsonOutput', 'checkTestFailed'],
            review: ['express:dev', 'protractor'],
    ...
    grunt.registerTask('dist', ['test', /*'build:dist',*/ 'jsdoc:dist']);
